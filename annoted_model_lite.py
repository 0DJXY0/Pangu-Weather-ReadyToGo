...
def gen_mask(x_shape,window_size) -> torch.Tensor:
  ...
  w_slices = (slice(0, -window_size[2]),
              slice(-window_size[2], -window_size[2]//2),
              slice(-window_size[2]//2, None))
  cnt = 0
  for z in z_slices:
    for h in h_slices:
        for w in w_slices:
...
# TODO
  ...
  print(x_shape)
  print(window_size)
  mask_windows=mask_windows.contiguous().view(1,x_shape[1]//window_size[0],x_shape[2]//window_size[1],x_shape[3]//window_size[2],-1) #[1,4,31,30,144]
  print(mask_windows.shape)
